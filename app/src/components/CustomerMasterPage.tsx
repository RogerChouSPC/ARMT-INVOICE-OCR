...
// ── Seed data ─────────────────────────────────────────────────────────────────
// Seed rows live in src/config/customerMasterSeed.ts (pure data, shared with the
// eval harness in scripts/eval).
function seedWithIds(): CustomerRow[] {
  return CUSTOMER_MASTER_SEED.map((r, i) => ({ ...r, id: `seed-${i}` }))
}
