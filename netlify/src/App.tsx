...
export default function App() {
  ...
  const processFiles = useCallback(async (files: File[]) => {
    ...
    for (let i = 0; i < files.length; i++) {
      const file = files[i]
      try {
        // ── Step 1: Try direct text extraction (digital PDFs — zero OCR needed)
        ...
        if (isDigital) {
          // ── Path A: Digital PDF → send text straight to Gemini for field extraction
          ...
          // ── Path B: Scanned/image PDF → render pages → Gemini Vision OCR
          ...
          const extractRes = await fetch('/.netlify/functions/extract', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              text: ocrTexts.join('\n\n--- PAGE BREAK ---\n\n'),
        ...
        const fileRows: InvoiceRow[] = (extractedRows as Partial<InvoiceRow>[]).map((r) => ({
          ...EMPTY_ROW(),
          ...r,
          seq: 0, // assigned below after we know total count
        }))

        if (fileRows.length === 0) {
          fileRows.push({ ...EMPTY_ROW(), remark: file.name })
        }
    ...
    // Append to existing rows with correct seq numbers (based on live state)
    setRows((prev) => [
      ...prev,
      ...newRows.map((r, i) => ({ ...r, seq: prev.length + i + 1 })),
    ])
      ...
      <main className="flex-1 max-w-screen-2xl mx-auto w-full px-6 py-8 flex flex-col gap-6">
        {rows.length === 0 && statuses.length === 0 && (
          <div className="text-center py-4 animate-fade-in">
            <h2 className="text-2xl font-medium text-gray-800">Extract Thai Invoice Data</h2>
            <p className="text-sm text-gray-500 mt-1">Powered by Google Gemini · 22-column Excel output</p>
          </div>
        )}
        ...
        {rows.length > 0 && (
          <>
            ...
            <div className="flex items-center justify-between">
              <button
                className="btn-secondary text-google-red border-red-200 hover:bg-red-50 hover:border-google-red"
                onClick={clearAll}
              >
